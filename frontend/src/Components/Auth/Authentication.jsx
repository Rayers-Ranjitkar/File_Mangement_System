...
const Authentication = ({ isLoginPage, onFormSubmit }) => {
  ...
  const [confirmPassword, setConfirmPassword] = useState(); //For signUp page
    ...
    <>
      <form className="font-quicksand">
        ...
        <div className="min-h-screen flex items-center  flex-col gap-4 justify-center px-20">
          {/* Upper Div */}
          <div className=" relative flex border-1 border-[#7051EF] flex-wrap flex-col gap-4 min-w-[80vw] sm:min-w-[50%] 2xl:min-w-[40%] px-6 p-2 py-6 items-center ">
            ...
            <div className=" min-w-[90%] sm:min-w-[45%] 2xl:min-w-[60%] relative ">
              ...
              />
              {/* Password Img span */}
              <span className="absolute z-10 top-[13%] right-3.5 2xl:right-3.5 text-3xl text-[#6b6767] ">
                {!isTypingPassword && (
                  <img
                    src={IconPassword}
                    alt="Password Icon"
            ...
            <div
              onClick={() => {
                onFormSubmit(email, password, confirmPassword); //Let's not overcomplicate for the project//we wanna clear up the fields so we can't call it here as we can't store the repo, we wanna store this reponse and if respose = success then, only we wanna clear up the fields so, we will have to create another form submit function and call it there
              }}
              className="flex h-10 2xl:h-12 mt-3.5 bg-[#7051EF]  rounded-md justify-center cursor-pointer font-bold text-xs 2xl:text-base items-center min-w-[90%] sm:min-w-[45%] text-white 2xl:min-w-[60%]"
            >
          ...
          {/* Lower Div   */}
          <div className="min-h-[10vh]  min-w-[80vw]  sm:min-w-[50%] 2xl:min-w-[40%] px-6 flex justify-center items-center gap-0.5 sm:gap-4 2xl:gap-6 text-sm 2xl:text-lg border-1 border-[#7051EF] ">
